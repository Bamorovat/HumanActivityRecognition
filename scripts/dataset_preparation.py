...
class VideoDataset(Dataset):
    ...
    def check_preprocess(self):
        if os.path.exists(os.path.join(self.output_dir, 'train')) and os.path.exists(os.path.join(self.output_dir, 'test')) and os.path.exists(os.path.join(self.output_dir, 'val')):
    ...
    def preprocess(self):
        ...
        if self.split == 'train':
            for video in train_video_files:
                ...
                if not os.path.exists(os.path.join(self.output_dir, 'train', file)):
                    os.mkdir(os.path.join(self.output_dir, 'train', file))
                self.process_video(videoo, file, os.path.join(self.output_dir, 'train', file))
        elif self.split == 'val':
            for video in val_video_files:
                file = format(video.split('/')[0])
                videoo = format(video.split('/')[1])
                if not os.path.exists(os.path.join(self.output_dir, 'val', file)):
                    os.mkdir(os.path.join(self.output_dir, 'val', file))
                self.process_video(videoo, file, os.path.join(self.output_dir, 'val', file))
        elif self.split == 'test':
            for video in test_video_files:
                file = format(video.split('/')[0])
                videoo = format(video.split('/')[1])
                if not os.path.exists(os.path.join(self.output_dir, 'test', file)):
                    os.mkdir(os.path.join(self.output_dir, 'test', file))
